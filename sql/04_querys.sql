...
WHERE CAL.DATA_TRANSACAO >= DATEADD(DAY,-90,GETDATE())
...
AND TEM.MES = GETDATE() 
----------------------------------------------------------
-- MEDIA CLIENTE MES/ANO
WITH SALDO_CONSOLIDADO(
        SELECT FS.ID_CLIENTE,
                EXTRACT(YEAR FROM FS.DATA_SALDO) AS ANO,
                EXTRACT(MONTH FROM FS.DATA_SALDO) AS MES,
                AVG(FS.SALDO) AS SALDO_MEDIO,
        FROM FATO_SALDO FS 
        GROUP BY   FS.ID_CLIENTE,EXTRACT(YEAR FROM FS.DATA_SALDO), EXTRACT(MONTH FROM FS.DATA_SALDO),        
)
...
WITH PIX_CONSOLIDADO(
        ...
        GROUP BY  T.ID_CLIENTE,DATEPART(YEAR,CAL.DATA_TRANSACAO),DATEPART(MONTH,CAL.DATA_TRANSACAO)
)
2.1 CONSULTA 
SELECT C.NOME AS CLIENTE,
       S.ANO,
...
WHERE CAL.DATA_TRANSACAO BETWEEN BG.DATA_INICIO AND COALASCE(DATA_FIM,'3000-12-31')
------------------------------------------------------------------
---28 GRUPO ECONOMICO 
